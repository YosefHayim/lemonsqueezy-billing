...
import type { BillingLogger, LoggerConfig } from "./types.js";
...
export function createLogger(config?: LoggerConfig): BillingLogger | undefined {
  if (!config) return undefined;
  if ("custom" in config && config.custom) return config.custom;
  if ("filePath" in config && config.filePath) return createFileLogger(config.filePath);
  return undefined;
}
...
export function withLogger<TArgs extends Record<string, unknown>, TResult>(
  ...
  return async (args: TArgs): Promise<TResult> => {
    ...
    try {
      ...
      logger.info({ ts: new Date().toISOString(), op, status: "ok", durationMs, input: sanitize(args), output });
      return result;
    } catch (err) {
      const durationMs = Date.now() - start;
