...
function cleanup(cachePath: string, logPath: string): void {
...
async function validateOffline(cachePath: string, logPath: string): Promise<void> {
  console.log("\n\ud83d\udce6 Offline Validation (no API key needed)\n");

  const { readCache, writeCache, isCacheValid } = await import("@core/cache.js");
  const { createWebhookVerifier, createWebhookHandler } = await import("@core/webhook.js");
  const { createLogger, withLogger } = await import("@core/logger.js");
  ...
  if (!isCacheValid(readCache(cachePath), 3_600_000)) pass("Cache TTL (expired rejected)");
...
async function validateLive(apiKey: string | undefined, cachePath: string, logPath: string): Promise<void> {
  if (!apiKey) {
    console.log("\n\ud83c\udf10 Live Validation\n");
